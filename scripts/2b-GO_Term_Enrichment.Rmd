...
Run GO and KO (KEGG Orthology) Term enrichment analysis using results from WGCNA analysis (i.e., after running the `2a-WGCNA.Rmd` script).
...
```{r variables}
# Input WGCNA results + GO and KO annotations
treatmentinfo.file <- "salmon.numreads.WGCNA_samples.tsv"
wgcna_results.file <- "salmon.numreads.WGCNA_results.tsv"
annotation.file <- "Gsulp_5572.genes.annots.tsv"

# Output DEG results
GO_term_sig.file <- "salmon.numreads.WGCNA_results.GOsig.tsv"
KEGG_sig.file <- "salmon.numreads.WGCNA_results.KEGGsig.tsv"

# Delimiter between GO and KO identifiers
GO.annot.split <- ","
KO.annot.split <- ","

# List of (one or multiple) modules that we want to perform enrichment analysis on (compared to all genes)
#  - Performs analysis on all genes in provided modules at the same time, not each module separately.
modules.to.analyze <- c("red", "royalblue")

# Cutoff for significant GO and KO term p-values
...
wgcna_results <- wgcna_results %>%
  filter(moduleColor %in% modules.to.analyze)
head(wgcna_results)
```
...
d1 <- dim(Go.ref); print(paste("Diff. Expr. Genes - rows:",d1[[1]]," columns:",d1[[2]], sep=''))
```


Set ID and gene length vectors, and make a binary matrix indicating which genes are differentially expressed. These are used as input to nullp, which calculates a Probability Weighting Function for DEGs.
...
wgcna.genes <- as.integer(IDvector %in% Go.ref$gene_id)
names(wgcna.genes) <- IDvector
...
splitted <- strsplit(as.character(GO.annot$GO_IDs), GO.annot.split) # Split into multiple GO ids
GO.terms <- data.frame(v1 = rep.int(GO.annot$gene_id, sapply(splitted, length)), v2 = unlist(splitted)) # List all genes with each of their GO terms (one per row)
colnames(GO.terms) <- c("gene_id", "GO.ID")

# Cleanup single-line GO term data.frame
GO.terms <- GO.terms %>%
...
GOall.filtered <- GOall %>% 
  filter(over_represented_pvalue < GO_enrich_pvalue.cutoff | under_represented_pvalue < GO_enrich_pvalue.cutoff) %>% # Filter GO enrichment results by p-value (over or under)
...
View and correct (if you wish) any un-annotated terms/ontologies.
```{r list_unannotated}
...
mappedIds <-function(df, collection, OFFSPRING) { # The command to run requires a data.frame of slim terms, like slims_MF above, your list of query terms, and the offspring from the GOCollection by goSlim
  map <- as.list(OFFSPRING[rownames(df)]) # Subset GOcollection offspring by the rownames of your data.frame
...
MFslimX <- unique(setDT(MFslimX)[order(go_term, -Count)], by = "go_term")  # Remove duplicate offspring terms, keeping only those that appear in the larger umbrella term (larger Count number)
  ...
  ggplot(aes(x = dir, y = term)) + 
    ...
    theme(panel.border = element_blank(),
          ...
          strip.text.y = element_text(angle=0,
                                      size = 3, # Size of text in boxes on right side
                                      face = "bold"),
          strip.text.x = element_text(size = 12, face = "bold"),
          axis.title.x = element_blank(),
          axis.title.y = element_text(size=15),
          axis.text = element_text(size = 1), # Size of text on the left side of plot
  ...
  ggplot(aes(x = dir, y = term)) + 
    ...
    theme(panel.border = element_blank(),
          ...
          strip.text.y.left = element_text(angle=0,
                                           size = 6, # Size of text in boxes on left side
                                           face = "bold"),
          strip.text.x = element_text(size = 12, face = "bold"),
          axis.title = element_blank(),
          axis.text = element_text(size = 4), # Size of text on the right side of plot
...
Select KEGG Orthogroup IDs (KOs) from annotation data.frame.
```{r extract_KO_annots}
...
splitted <- strsplit(as.character(KO.terms$KEGG_IDs), KO.annot.split) # Split into multiple KEGG IDs
...
colnames(KO.terms) <- c("gene_id", "KEGG.IDs")
head(KO.terms)
```
...
```{r goseq_on_KOs}
KOall <- goseq(pwf, GOref$gene_id, gene2cat=KO.terms, test.cats=c("KEGG"), method="Wallenius", use_genes_without_cat=TRUE)
```
...
KOall.filtered <- KOall %>% 
  ...
  arrange(over_represented_pvalue, -numDEInCat) %>% # Reorder
  mutate(dir = if_else(over_represented_pvalue < GO_enrich_pvalue.cutoff, 
                       "Over", 
                       if_else(under_represented_pvalue < GO_enrich_pvalue.cutoff, 
                               "Under", 
