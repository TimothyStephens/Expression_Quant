...
condition <- "groupName"
treatments.to.compare <- c("CTRL_TP10", "Glu_D_TP10")
treatment.colors <- c("CTRL_TP10" = "#1b9e77", "Glu_D_TP10" = "#d95f02")
...
Filter `treatmentinfo` to just the samples that we want to analyze/compare.
<span style="color:red">**Adjust filtering as required.**</span>
```{r filter_treatmentinfo}
treatmentinfo <- treatmentinfo %>%
  filter(!!rlang::sym(condition) %in% treatments.to.compare) %>% # Filter
  mutate({{condition}} := factor(!!rlang::sym(condition), levels = treatments.to.compare)) # Make factor for plotting
head(treatmentinfo)
```
...
print(paste("Num DEGs:", nrow(DEGs), sep=''))
...
rownames(DEGs) <- NULL
DEGs <- relocate(DEGs, "gene_id", .before="baseMean") # Make first column
...
# If the above max(size factors) is less than 4, we can use VST.
...
```{r plot_heatmap}
hmTreatment <- subset(treatmentinfo, select=c(condition)) # Get just the condition column
col <- list(); col[[condition]] = treatment.colors # Setup list for heatmap colors
hm_ann_col <- HeatmapAnnotation(df=hmTreatment, col = col) # Make data.frame for column naming
...
DEG_PCA <- ggplot(data = d, aes(x = PC1, y = PC2)) +
  geom_point(size = 2, # Point size
             aes(colour=!!rlang::sym(condition))) +
...
DEG_PCA
ggplotly(DEG_PCA, width = 1000, height = 1000)
```
