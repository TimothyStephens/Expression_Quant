---
title: "DGE GO Term and KEGG Enrichment Analysis"
...
Run GO and KO (KEGG Orthology) Term enrichment analysis using results from DGE analysis (i.e., after running the `1a-DGE.Rmd` script).
...
```{r variables}
# Input DEG results + GO and KO annotations
...
# KEGG ID description file - usually provided with script
KEGG_IDs_Descriptions.file <- "1b-KEGG_IDs_Descriptions.tsv.gz"
```
...
dge.genes <- as.integer(IDvector %in% Go.ref$gene_id)
names(dge.genes) <- IDvector
  ...
  ggplot(aes(x = dir, y = term)) + 
    ...
    theme(panel.border = element_blank(),
          ...
          strip.text.y = element_text(angle=0,
                                      size =2, # Size of text in boxes on right side
                                      face = "bold"),
...
MFplot <- GO.MF %>%
  ...
  ggplot(aes(x = dir, y = term)) + 
    ...
    theme(panel.border = element_blank(),
          ...
          axis.text = element_text(size = 1), # Size of text on the right side of plot
...
Select KEGG Orthology IDs (KOs) from annotation data.frame.
```{r extract_KO_annots}
